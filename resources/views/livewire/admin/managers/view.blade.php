...
<x-slot name="breadcrumb">
    ...
    <li aria-current="page">
        <div class="flex items-center">
            <svg aria-hidden="true" class="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
            <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">{{ __('Managers') }}</span>
        </div>
    </li>
</x-slot>
...
<div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
    <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-4 dark:bg-gray-800">
        ...
        <button wire:click="create()" class="bg-blue-500 hover:bg-blue-700 text-white py-1 mb-6 px-3 rounded my-3 mt-1">Create new manager</button>

        @if($isModalOpen)
            @include('livewire.admin.managers.modalform')
        @endif
        ...
        @if (count($managers) > 0)
            <table class="table-fixed w-full">
                ...
                @foreach($managers as $manager)
                    <tr>
                        <td class="border dark:border-gray-600 dark:text-gray-400 px-4 py-2">{{ $manager->id }}</td>
                        <td class="border dark:border-gray-600 dark:text-gray-400 px-4 py-2">{{ $manager->name }}</td>
                        <td class="border dark:border-gray-600 dark:text-gray-400 px-4 py-2">{{ $manager->email }}</td>
                        <td class="border dark:border-gray-600 dark:text-gray-400 px-4 py-2 text-center">
                            <button wire:click="edit({{ $manager->id }})" class="bg-amber-500 hover:bg-amber-700 text-white py-1 px-3 rounded">{{ __('Edit') }}</button>
                            <button wire:click="confirmItemDeletion({{ $manager->id }})" wire:loading.attr="disabled" class="bg-red-500 hover:bg-red-700 text-white py-1 px-3 rounded">{{ __('Delete') }}</button>
                        </td>
                    </tr>
                @endforeach
                </tbody>
    ...
    {{ $managers->links('pagination', ['is_livewire' => true]) }}
