@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="px-5 py-5 bg-white border-t flex flex-col xs:flex-row items-center xs:justify-between ">
        ...
        <div class="px-5 py-5 bg-white flex flex-col xs:flex-row items-center xs:justify-between">
            <div>
                <p class="text-xs xs:text-sm text-gray-900">{!! __('Exibindo de') !!} <span class="font-medium">{{ $paginator->firstItem() }}</span> {!! __('até') !!} <span class="font-medium">{{ $paginator->lastItem() }}</span> {!! __('do total de') !!} <span></span><span class="font-medium">{{ $paginator->total() }}</span> {!! __('registros') !!}</p>
            </div>
